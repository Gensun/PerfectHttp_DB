...
let mysql_user = "root"
let mysql_pwd = "12345678"
let mysql_db = "EF"

//MARK: 表信息
let userTable = "user" // 用户信息表
...
open class DataBaseManager {
    ...
    private func connectedDataBase() -> Bool {
        ...
        let connected =  mysql.connect(host: mysql_host, user: mysql_user, password: mysql_pwd, db: mysql_db)
    ...
    func mysqlStatement(_ sql: String) -> (success: Bool, mysqlResult: MySQL.Results?, errorMsg: String) {
        ...
        guard mysql.selectDatabase(named: mysql_db) else {            //指定database
            let msg = "未找到\(mysql_db)数据库"
            print(msg)
